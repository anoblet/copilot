...
function assertSymlink(filePath: string, expectedTarget: string) {
...
function cleanupTempWorkspace(root: string) {
...
function testMixedArraySchema() {
  ...
  try {
    ...
    assertSymlink(path.join(destDir, '.github', 'extra', 'extra.md'), '../../../src/extra.md');

    assertSymlink(path.join(destDir, '.vscode', 'mcp.json'), '../../src/mcp.json');
    assertSymlink(path.join(destDir, '.vscode', 'settings.json'), '../../src/settings.json');
  } finally {
...
function testLegacyObjectSchema() {
  ...
  try {
    ...
    assertSymlink(path.join(destDir, 'legacy', 'sub', 'a.txt'), '../../src/a.txt');
    assertSymlink(path.join(destDir, 'legacy', 'sub', 'b.txt'), '../../src/b.txt');
  } finally {
    cleanupTempWorkspace(root);
  }
}

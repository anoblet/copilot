...
- Room-specific Configuration views in the Rooms dashboard should live at `includes/lovelace/views/<area>/configuration.yaml` with `subview: true`, use `type: sections` or omit `type` when relying on the default, declare their content under a top-level `sections:` list instead of a top-level `cards:` list, group surfaced tiles into concern-based section blocks, use multiple sections when multiple concerns are surfaced, and allow a single named section when only one concern is present. Use built-in card types, give each surfaced entity its own built-in `tile` card, omit the area name from labels, include only entities created or configured by YAML packages under `packages/`, be included from `includes/lovelace/dashboards/dashboard_room.yaml`, and be opened from a dedicated Configuration tile in the area view with `navigation_path: /dashboard-room/<area>-configuration`. Keep that Configuration tile as the last item on its row in the area view.
